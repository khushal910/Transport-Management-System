  ...
  if (!mounted) {
    return (
      <div className="py-8 text-center">
        ...
        <p className="mt-3 text-sm font-medium text-slate-600">Loading setup page...</p>
      </div>
    );
  }
  ...
  if (mounted && !token) {
    return (
      <div className="space-y-4 text-center">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-950">Invalid Setup Link</h2>
          <p className="mt-2 text-sm text-slate-600">The setup link is invalid or has expired.</p>
        </div>
        <button
  ...
  if (isSetup) {
    return (
      <div className="space-y-6 text-center">
        ...
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-emerald-700">
            <span className="h-2 w-2 rounded-full bg-emerald-600" />
            Account Setup Complete
          </div>
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-950">Welcome to FleetFlow</h1>
          <p className="mt-2 text-sm text-slate-600">
            Your password has been set successfully. Your account is now active and ready to use.
          </p>
        </div>

        <button
          onClick={() => navigate('/auth/login')}
          className="group inline-flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-r from-blue-600 to-indigo-600 px-8 py-3 font-semibold text-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md"
        >
          Proceed to Login
