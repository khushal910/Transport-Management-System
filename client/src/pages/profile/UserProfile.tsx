 ...
 * Displays personal, company, and driver-specific details
 */
export const UserProfilePage: React.FC = () => {
  ...
  const handleVerifyEmailChange = async () => {
    ...
    try {
      ...
      if (updatedProfile && updatedProfile.personal.name !== trimmedName) {
        const finalProfile = await updateUserProfile({ name: trimmedName });
  ...
  const handleSaveCompany = async () => {
    ...
    if (trimmedName !== profile.company.name) updatePayload.name = trimmedName;
    if (trimmedAddress !== profile.company.address) updatePayload.address = trimmedAddress;
    if (trimmedPhone !== profile.company.phone) updatePayload.phone = trimmedPhone;
    if (trimmedEmail !== profile.company.email) updatePayload.email = trimmedEmail;
            ...
            {[1, 2, 3].map((item) => (
              <div key={item} className="mb-6">
                ...
                <div className="space-y-2">
                  {[1, 2, 3].map((sub) => (
                    <div key={sub} className="h-4 w-full bg-gray-300 rounded animate-pulse" />
                  ))}
                </div>
              </div>
            ))}
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">View personal, company, and driver details</p>
        </div>

        <div className="bg-white rounded-lg shadow-lg p-8 mb-6">
          <div className="flex flex-col lg:flex-row items-start gap-6 mb-8 pb-8 border-b border-gray-200">
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                ...
                <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg font-medium ${currentRole.color}`}>
              ...
              <div className="mt-4 text-sm text-gray-600">
                ...
                {personal.isPasswordSet ? (
                  ...
                  <p className="text-amber-600 mt-1">Password not set</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-gray-900">{currentRole.label}</p>
              </div>
              <div>
            ...
            {isVerifyingEmail ? (
              <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-5">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-amber-900">Verify your new email</h4>
                    <p className="text-sm text-amber-800">A code was sent to <strong>{pendingNewEmail}</strong>. Enter it below to confirm the email change.</p>
                  </div>
                </div>
                {otpError ? (
                  <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button
                    ...
                    disabled={otpLoading}
                    className="inline-flex items-center gap-2 rounded-lg bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors duration-200"
                  >
                    {otpLoading ? (
                      <>
            ...
            {isEditing && !isVerifyingEmail ? (
              <div className="mt-6 flex flex-wrap gap-3">
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-3 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors duration-200"
                >
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <p className={`font-semibold ${company.status === 'active' ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="bg-linear-to-br from-blue-50 to-blue-100 rounded-lg p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white rounded-lg p-4">
                    ...
                    <p className="text-gray-900 font-semibold text-lg capitalize">{driver.licenseCategory}</p>
                  </div>
                  <div className="bg-white rounded-lg p-4">
                    <label className="block text-sm font-medium text-gray-700 mb-2">License Expiry</label>
                    <p className="text-gray-900 font-semibold">{new Date(driver.licenseExpiry).toLocaleDateString()}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                  <div className="bg-white rounded-lg p-4">
                    ...
                    <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${driver.safetyScore >= 80 ? 'bg-green-500' : driver.safetyScore >= 60 ? 'bg-yellow-500' : 'bg-red-500'}`}
                  ...
                  <div className="bg-white rounded-lg p-4">
                    ...
                    <p className="text-gray-900 font-semibold text-xs">Completed: <span className="text-green-600">{driver.completedTrips}</span></p>
                    <p className="text-gray-900 font-semibold text-xs">Assigned: <span className="text-blue-600">{driver.assignedTrips}</span></p>
                  </div>
                </div>
              </div>
            </div>
